{{-- this is the view for the notes page --}}
            ...
            <div class="flex">
                ...
                @else
                    ...
                    <form action="{{ route('trashed.destroy', $note) }}" method="post">
                        @method('delete')
                        @csrf
                        <button type="submit" class="btn btn-danger ml-4" onclick="return confirm('Are you sure you want to permanently delete this note?')">Permanently Delete</button>
                    </form>
                @endif
            </div>
            <div class="my-6 p-6 bg-white border-b border-gray-200 shadow-sm sm:rounded-lg">
                <h2 class="font-bold text-4xl" mt-6>
                    {{ $note->title }}
                </h2>
